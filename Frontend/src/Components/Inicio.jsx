  ...
  const obtenerDatos = async () => {
    try {
      ...
      const [susRes, gastosRes, balRes] = await Promise.all([
        axios.get(`http://localhost:3000/auth/suscripciones/usuario/${userId}`),
        axios.get(`http://localhost:3000/auth/total-gastos/usuario/${userId}`),
        axios.get(`http://localhost:3000/auth/balance/usuario/${userId}`)
      ]);
